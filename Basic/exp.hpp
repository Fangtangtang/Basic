...
enum ExpressionType {
    CONSTANT, IDENTIFIER, COMPOUND//常数 标识符 复合
};
 ...
 * objects of its own.
 * 定义结构和类的行为 不含对象
 * Any object must be one of the three
 * concrete subclasses of Expression:
 *三个派生
 *
 *  1. ConstantExp   -- an integer constant//整型常数
 *  2. IdentifierExp -- a string representing an identifier//代表标识符的字符串
 *  3. CompoundExp   -- two expressions combined by an operator//两个表达式 一个操作符
 *
 * The Expression class defines the interface common to all
 * Expression objects;
 * each subclass provides its own specific implementation of the common interface.
 *
...
/*
 * 表达式类型 CONSTANT, IDENTIFIER, or COMPOUND
 * Method: type
