 ...
 * 储存程序的行
 * Each line in the program is stored in order according to its line number.
 * 有序行号开始
 ...
 */
    ...
    //more func to add
    //todo
   void runFrom(int lineNumber,EvalState &state);
   int next(int lineNumber);
   void setGoTo(int lineNumber,int go);
...
private:
    ...
    struct lineInformation{
        std::string text;
        Statement *statement=nullptr;//行状态
        int goTo=0;
    };
    std::unordered_map<int,lineInformation> aLine;//行号到行信息
    std::set<int> lineNum;//行号的有序表
};
